...
/// <summary>
/// DEMO 4 — MCP Apps : widget UI interactif dans le client
/// 
/// Principe :
///   1. Déclarer une Resource avec MimeType = "text/html;profile=mcp-app" et URI ui://
///   2. Retourner du HTML auto-contenu (CSS + JS inline)
///   3. Le JS communique avec le serveur MCP via postMessage JSON-RPC
///   4. Le client affiche l'HTML dans un iframe sandboxé
/// 
/// ⚠️ Status (avril 2026) : preview dans le SDK C#
///     Vérifier la dernière API dans modelcontextprotocol/csharp-sdk le matin même
/// </summary>
[McpServerResourceType]
public class DashboardUiResource
{
    /// <summary>
    /// Resource UI : déclarée avec le MimeType spécial "text/html;profile=mcp-app"
    /// URI scheme : ui://
    /// </summary>
    [McpServerResource(
        Uri = "ui://belgabrew/inventory-dashboard",
    ...
    public string GetDashboardHtml()
    {
        ...
        <script>
        // Communication avec le MCP Server via postMessage JSON-RPC
        // C'est le mécanisme MCP Apps : l'iframe appelle des tools côté serveur
        async function callTool(name, args) {
          return new Promise((resolve) => {
            ...
            // JSON-RPC 2.0 — même protocole que le reste du serveur MCP
        ...
        async function render() {
          ...
          const results = await Promise.all(
            skus.map(sku => callTool('check_ingredient_stock', { sku }))
          );
        ...
        async function reorder() {
          await callTool('place_reorder', { auto: true });
